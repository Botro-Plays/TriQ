...
| 19 | **Prisma Client generation on VPS** — `email` field missing in generated client, seed fails | **Packaged root `node_modules` from GitHub Actions** — includes generated client; VPS only extracts, no `npm install` needed. Also copies `.prisma/client` into `@prisma/client/.prisma/client` for correct resolution | `.github/workflows/deploy-vps.yml`, `apps/server/package.json` |
| 20 | **dotenv not loading under PM2** — `DATABASE_URL` not found because CWD is `dist/` | **Use `path.join(__dirname, '..', '.env')`** — always resolves to project root regardless of PM2 CWD | `apps/server/src/index.ts` |
| 21 | **Google Sign-In missing** — only Phone OTP supported, DITO carrier issues | **Added Google Sign-In** as alternative; extracts email from Firebase token; supports account linking by phone; owner claiming via Google | `apps/web/src/pages/Login.tsx`, `apps/server/src/routes/auth.ts` |
| 22 | **Owner account creation** — anyone could select OWNER role, no ownership protection | **Seeded owner** with placeholder `firebaseUid='OWNER_PENDING'`; claiming links Google account; prevents duplicate OWNER; hides OWNER button after claimed | `apps/server/prisma/seed.ts`, `apps/server/src/routes/auth.ts`, `apps/web/src/pages/Login.tsx` |
